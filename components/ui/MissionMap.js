...
export default function MissionMap({ missions = [] }) {
  ...
  // 🔹 Compute circular positions dynamically
  const nodePositions = missions.map((_, i) => {
    const angle = (i / missions.length) * 2 * Math.PI;
    return {
  ...
  return (
    <div className="relative flex items-center justify-center w-full h-[650px] bg-gradient-to-b from-[#050510] to-[#090821] rounded-2xl border border-green-500 shadow-lg overflow-hidden">
      ...
      {/* 🌌 Single glowing orbit ring */}
      

      {/* 🟢 Animated lines between missions (simple smooth circle) */}
      ...
      {missions.map((mission, i) => {
        ...
        const isCompleted =
          ...
          localStorage.getItem(`mission-jenkins-${mission.id}`) === "completed";
        ...
        return (
          <motion.div
            ...
            onClick={() => router.push(`/mission/jenkins/${mission.id}`)}
          >
            <motion.div
